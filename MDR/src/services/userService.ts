...
export default class UserService implements IUserService {
  ...
  public async updateUser(user: IUserDTO, email: string): Promise<Result<IUserDTO>> {
    try {
      ...
      const userOrError = await User.create(user);
      ...
      //update user
      const updatedUser = await this.userRepo.update(userDomain, email);

      const userDTO = UserMap.toDTO(userDomain) as IUserDTO;
